...
-- table making for male and female state wise
-- formaula to find count of male and female
-- male =population/(1+sex_ratio)   , female = (population * sex_ratio)/(1 + sex_ratio)

drop table if exists #fmcount
create table #fmcount(
state nvarchar(255),
male_count int ,
female_count int
)
Insert into #fmcount
select fm.state , sum(fm.male) Male_Count ,sum(fm.female) Female_Count from 
(select district ,state  , round(( Population/1+(sex_ratio) ) , 0) Male ,
round(((population * sex_ratio)/(1+sex_ratio)) , 0) as Female from #joiningtable)  fm
group by fm.State;
